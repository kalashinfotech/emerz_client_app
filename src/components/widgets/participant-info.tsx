...
export const ParticipantInfoWidget = () => {
  ...
  return (
    <Card className="from-primary-300 to-primary-600 mx-auto w-80 rounded-2xl bg-radial-[at_50%_85%] to-90% px-4 py-4 md:mx-0">
      <CardHeader>
        <CardDescription className="flex items-center gap-2 text-base text-white">
          {sessionInfo.loginCount === 1
            ? `Welcome aboard! Let’s get started on something amazing.`
            : `Back for more? Let’s keep the momentum going!`}
        </CardDescription>
        <CardTitle className="text-4xl font-medium text-white">{sessionInfo.firstName}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <CardDescription className="text-muted flex gap-3 text-xs">
        ...
        <div className="grid grid-cols-2">
          {sessionInfo.verifiedDate && (
            <div>
              <CardDescription className="text-muted text-xs">Joined on</CardDescription>
              <CardDescription className="text-xs text-white">
                {format(sessionInfo.verifiedDate, 'dd MMM, yyyy')}
              </CardDescription>
            </div>
          )}
          <div>
        ...
        {sessionInfo.batch && (
          <div className="grid grid-cols-1">
            <div>
              <CardDescription className="text-muted text-xs">Institution</CardDescription>
              <CardDescription className="text-xs text-white">{sessionInfo.batch.institution.name}</CardDescription>
              <CardDescription className="text-xs text-white">{sessionInfo.batch.name}</CardDescription>
            </div>
          </div>
        )}
      </CardContent>
      <CardFooter className="text-muted flex-col items-start text-xs">
        {perc === 100 ? (
        ...
        <div className="flex w-full items-center gap-4 text-xs">
          <Progress className="bg-white/50" indicatorClassName="bg-white" value={perc} />
